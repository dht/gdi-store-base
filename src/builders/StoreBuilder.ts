...
import { Middleware, applyMiddleware, combineReducers, compose, createStore } from 'redux';
...
export class StoreBuilder implements IStoreBuilder {
  ...
  withMiddlewares(middlewares: Middleware | Middleware[]) {
    const extraMiddlewares = Array.isArray(middlewares) ? middlewares : [middlewares];

    this.definition.middlewares = [...this.definition.middlewares, ...extraMiddlewares];
  ...
  withEnhancers(enhancers: any | any[]) {
    ...
    this.definition.enhancers = [...this.definition.enhancers, ...extraEnhancers];
  ...
  hotReplaceRootSage = (root: any) => {
    const methodName = findSagaMethodName(root);

    if (!methodName) {
      console.log(root);
      throw new Error('No root saga method found');
    }

    const task = this.tasks[methodName];

    if (!task) {
      console.log(`No task found for ${methodName}`);
    }

    task.cancel();

    this.tasks[methodName] = this.sagaMiddleware.run(root);
  };
  ...
  build<T = any>(): T {
    ...
    const composedEnhancers = composeMethod(middlewareEnhancer, ...this.definition.enhancers);

    const store = createStore(rootReducer, this.definition.initialState, composedEnhancers) as any;
    ...
    this.definition.sagas.forEach((saga: any) => {
      const methodName = findSagaMethodName(saga);

      if (!methodName) {
        this.sagaMiddleware.run(saga);
      } else {
        if (this.tasks[methodName]) {
          throw new Error(`Saga ${methodName} already running`);
        } else {
          this.tasks[methodName] = this.sagaMiddleware.run(saga);
        }
      }
    });
...
export interface IStoreBuilder {
  ...
  build<T = any>(): T;
  hotReplaceRootSage: (root: any) => void;
}

export const findSagaMethodName = (saga: any) => {
  const methodNameRegex = /\* ([^(])+/;
  const matches = methodNameRegex.exec(saga.toString());
